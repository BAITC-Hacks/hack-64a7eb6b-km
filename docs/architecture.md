...
- `RuntimeConfiguration` разрешает `auto` в фактический `demo` / `laravel` до постановки задачи; `context.runtime` хранит причину выбора без ключа. Worker исполняет сохранённый режим. Принудительный `demo` блокирует ранее поставленные live-задачи; удаление ключа завершает live-задачу ошибкой, а новые задачи в `auto` получают демо. Ошибки провайдера не запускают fallback.
...
Аутентификация и email verification — Laravel Fortify. Inertia-routes требуют `auth,verified`. Для Filament нужны подтверждённый email и роль `super_admin` с guard `web`. `User::canAccessAdmin()` задаёт единое условие для панели, policies управления и просмотра всех запусков. `canAccessPanel()` дополнительно проверяет ID панели `admin`; auth middleware повторно выполняется на Livewire-запросах. Регистрация назначает только роль «Аким (Городской управленец)»; присланные клиентом роли и разрешения игнорируются.

В панели доступны пользователи, роли и каталог разрешений. `AccessPermission` содержит только действия фронтенда. `AccessControlSeeder` создаёт три стартовые роли: `super_admin` с доступом ко всем разделам панели и правами фронтенда, «Аналитик» с просмотром своих запусков и «Аким (Городской управленец)» с действиями над своими запусками. Все прочие роли и прямые разрешения пользователя действуют только на фронтенде. Повторный seed не сбрасывает настройки обычных ролей. «Аким (Городской управленец)» назначается при регистрации, через локальную CLI-команду без `--admin` и при создании пользователя без явного выбора ролей; эту роль нельзя удалить или переименовать, но её разрешения можно настроить. Каталог разрешений нельзя произвольно менять через UI: новые действия добавляются вместе с их серверной реализацией.
...
Миграция `replace_default_roles_with_city_roles` заменяет «Участник» на «Аким (Городской управленец)», а «Наблюдатель» на «Аналитик», сохраняя назначения и настройки прав. Если новая роль уже существует, назначения объединяются без дубликатов, её права сохраняются. Остальные роли guard `web` и их назначения удаляются, кроме `super_admin`; пользователи и их прямые разрешения сохраняются. Обратная миграция запрещена, поскольку удалённые назначения нельзя восстановить.

Права пользовательского приложения: `workspace.view` (свои запуски и заметки), `workspace.runs.create`, `workspace.runs.cancel`, `workspace.approvals.resolve`. Для любого действия нужно также право просмотра; write-права сами по себе не открывают рабочее пространство. По умолчанию «Аналитик» получает только `workspace.view`, а «Аким (Городской управленец)» — все четыре разрешения.
...
`simulation_datasets` содержит неизменяемый JSONB-снимок `astana-v1` и SHA-256 исходного JSON из `database/seeders/data`. `simulation_scenarios` принадлежит пользователю и хранит выбранные меры, результат, три альтернативы, версию калькулятора и ссылки на исходный сценарий/подтверждение. Изменение оформляется новым сценарием. Сравнение допускает только одинаковые версии данных и расчёта.

```mermaid
flowchart LR
    UI[React / Inertia] --> Validation[ValidateScenario]
    Validation --> Calculation[CalculateScenario · BCMath]
    Calculation --> Saved[(SimulationScenario)]
    Saved --> Facts[ScenarioFacts]
    Facts --> Queue[CreateScenarioRun / ExecuteAgentRun]
    Queue --> Analysis[ScenarioAnalysisAgent]
    Queue --> Chat[ScenarioChatAgent]
    Chat --> Broker[ToolBroker]
    Broker --> Evaluation[evaluate_scenario]
    Broker --> Proposal[(propose_scenario / ToolApproval)]
    Proposal --> Approval[Владелец подтверждает]
    Approval --> Variant[(Новый сценарий)]
```

`CalculateScenario` суммирует знаковые эффекты с множителем `(8−lag)/8`, добавляет фиксированные синергии, один раз ограничивает итоговые показатели диапазоном 0–100 и вычисляет Score. Промежуточные величины — BCMath с восемью десятичными знаками. Штраф применяется строго при значении ниже 40. UI округляет только отображение; сохранённые значения остаются точными.

`FindScenarioAlternatives` перебирает допустимые замены одной меры, включая смену её района. Возвращает максимум три положительных улучшения в порядке: прирост Score по убыванию, расходы по возрастанию, стабильный идентификатор. Отсутствие улучшений — допустимый результат, это не доказательство глобального оптимума.

`agent_runs.kind` различает `workspace`, `scenario_analysis` и `scenario_chat`. В очередь попадают снимки фактов, модели, лимитов и prompt version. Разбор возвращает структурированные summary/strengths/risks/tradeoffs/recommendations; ID рекомендаций проверяются против рассчитанных альтернатив. Диалог получает последние шесть успешных пар сообщений этого сценария, не более 24 000 символов. Одновременно допускаются один разбор и один диалог на сценарий, всего до трёх активных запусков на владельца.

Инструменты принимают только полный набор `selections`; пользователь, сценарий, датасет и формула выбираются сервером. Недопустимые комбинации расходуют tool budget и возвращают ошибки. `propose_scenario` сохраняет неизменяемый payload с точными решениями и расчётом; повторный одинаковый вызов в одном запуске не создаёт второй proposal. Только отдельный авторизованный запрос владельца после успешного завершения run создаёт вариант. Перед записью сервер повторяет расчёт и проверяет совпадение. Блокировки остаются в порядке run → approval; отменённые и неуспешные запуски ничего не применяют.

`SimulationScenarioResource` в Filament позволяет супер-администратору только просматривать снимки. Пользовательские endpoints не дают супер-администратору доступа к чужим сценариям. Страницы симулятора расположены в `resources/js/pages/scenarios`, общие таблицы и диаграммы — в `components/simulation`; публичный интерактивный прототип на `/` остаётся отдельным.

Демо-диалог формирует тематические ответы из snapshot фактов. `output_data.demo_context` хранит только упомянутые в конкретной реплике район и вариант; следующий запуск восстанавливает их из тех же последних шести пар, ограниченных 24 000 символов. Неоднозначные ссылки требуют уточнения. Обсуждение улучшений не создаёт proposal; явная просьба подготовки сначала проверяет вариант через `evaluate_scenario`, затем вызывает `propose_scenario`. Inertia передаёт `aiRuntime` и разрешённые поля событий действий, а внутренний контекст демо-диалога скрыт.
